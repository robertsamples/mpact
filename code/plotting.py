...
class plot_samplecorr(ui_plot):
    """
    Sample-correlation heatmap, with a Method/View/label switcher bar
    inserted into the *shared* Group Analysis nav bar (frame_12 /
    horizontalLayout_25, alongside the pre-existing "Sets"/"Sample
    Correlations" buttons) rather than this plot's own canvas -- unlike
    plot_dendrogram/plot_ordination's per-canvas bars, these controls only
    apply to this page, and the nav bar is shared with the UpSet plot page.
    UIFunctions.switch_grpanalysis_tab (ui_functions.py) greys the controls
    out when the UpSet page is active.

    Views (same collapsing semantics as plot_dendrogram/plot_ordination,
    via ordination.load_ordination_matrix):
    - "Biological Replicates" (default, matches this plot's previous,
      checkbox-less behaviour): technical replicates averaged per Sample,
      biological replicates kept separate.
    - "Individual Injections": no averaging, every injection is its own
      row/column.
    - "Biological Groups": both technical and biological replicates
      averaged together, one row/column per Biolgroup -- "see only
      biological groups" with technical-replicate averaging implied.

    Methods (ordination.similarity_matrix):
    - "Spearman" (default): rank correlation of abundance profiles.
    - "Jaccard": presence/absence (feature detected or not), ignoring
      abundance.
    - "Bray-Curtis": abundance-weighted, the standard ecology/metabolomics
      similarity measure.

    "Use Sample/Group Names" checkbox: same nomenclature as the
    dendrogram's -- ``<Biolgroup>_b<BioRep#>_s<TechRep#>`` (Individual
    Injections), ``<Biolgroup>_b<BioRep#>`` (Biological Replicates), or the
    raw Biolgroup name (Biological Groups -- nothing left to shorten).
    """

    VIEWS = ('Biological Replicates', 'Individual Injections', 'Biological Groups')
    METHODS = ('Spearman', 'Jaccard', 'Bray-Curtis')
    ...
    def _build_grpanalysis_controls(self, parent):
        ...
        # Sets/Sample Correlations buttons within the same shared bar,
        # rather than editing the generated horizontalLayout_25 itself.
        parent.ui.horizontalLayout_25.addStretch(1)
        parent.ui.horizontalLayout_25.addWidget(bar)

    def set_controls_enabled(self, enabled):
        """Grey the Method/View/Use-Names controls out when the UpSet Plot
        page (the nav bar's other tab) is active -- they don't apply there."""
    ...
    def _load_matrix(self, parent):
        ...
        x, biolgroup = ordination.load_ordination_matrix(
            pltfile, raw_header.copy(), collapse_replicates=(self.view != 'Individual Injections'))
        if self.view == 'Biological Groups':
            x = x.groupby(biolgroup).mean()
        return x, raw_header

    def _display_labels(self, raw_header, leaf_names):
        """Build short ``Biolgroup_b#[_s#]`` labels, mirroring
        plot_dendrogram's ``_display_labels`` -- nomenclature switches on
        the active View the same way."""
        if self.view == 'Biological Groups':
            return leaf_names  # already bare Biolgroup names
        ...
        if self.view == 'Biological Replicates':
            ...
            return [f"{per_sample.loc[sample, 'Biolgroup']}_b{per_sample.loc[sample, 'BioRep']}" for sample in leaf_names]
        return [
            f"{components.loc[injection, 'Biolgroup']}_b{components.loc[injection, 'BioRep']}_s{components.loc[injection, 'TechRep']}"
            for injection in leaf_names
        ]
    ...
    def plot(self, parent, file, filtereddfs, groupsets):
        ...
        x, raw_header = self._load_matrix(parent)
        pmatrix = ordination.similarity_matrix(x, self.method)

        leaf_names = pmatrix.columns.tolist()
        display_labels = self._display_labels(raw_header, leaf_names) if self.use_sample_names else leaf_names

        fig = self.parent.fig[self.currplt]
        # clf() + add_subplot: sns.heatmap permanently shrinks the axes each
        # call to make room for its colorbar; removing the extra colorbar axes
        # afterwards doesn't restore the original size. Starting fresh each
        # call keeps the axes at a consistent width.
        fig.clf()
        ax = fig.add_subplot(111)
        ax.set_facecolor(self.plotbackground)
        ax.set_axisbelow(True)
        self.parent.ax[self.currplt] = ax
        # Spearman is mathematically capable of going negative, but real
        # sample-vs-sample correlations in practice cluster tightly positive
        ...
        ax.set_title(self.method, fontsize=10)
...
class plot_dendrogram(ui_plot):
    ...
    - "Purity": green wherever a branch's leaves are entirely one group
      (correctly clustered), magenta wherever a branch mixes more than one
      group (polyphyletic). Green/magenta rather than the more conventional
      green/red since red-green colorblindness can't distinguish the latter.
    ...
    def plot(self, parent, file, filtereddfs, groupsets):
        ...
        if self.color_mode == 'Purity':
            # Green = monophyletic (correctly clustered); magenta =
            # polyphyletic (mixes more than one group). Magenta rather than
            # the conventional red -- distinguishable from green under
            # red-green colorblindness, the most common form.
            link_color_func = clusterpurity.purity_link_color_func(Z, leaf_labels, true_color='green', false_color='magenta')
        else:
